...
async function main() {
  ...
  console.log("通常の単一ベクトル(Dot Product)検索では欠落してしまう「細部の情報」を、");
  ...
  const docATokens = new Float32Array([
    0.85, 0.0, 0.0, 0.0,  // Apple
    0.0, 0.9, 0.0, 0.0,   // Orange
    0.0, 0.0, 0.0, 0.0    // (Bananaなし)
  ]);

  // ドキュメントB: "Banana is yellow. Apple is red."
  const docBTokens = new Float32Array([
    0.0, 0.0, 0.9, 0.0,   // Banana
    0.8, 0.0, 0.0, 0.0,   // Apple
    0.0, 0.0, 0.0, 0.1    // Grape
  ]);

  // ドキュメントC: "Orange and Grape are sweet."
  const docCTokens = new Float32Array([
    0.0, 0.8, 0.0, 0.0,   // Orange
    0.0, 0.0, 0.0, 0.9    // Grape
  ]);
  ...
  const results = adapter.rank(queryTokens, [docATokens, docBTokens, docCTokens], dim);
