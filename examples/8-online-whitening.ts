...
console.log("OpenAI ada-002 などのように、ベクトル空間全体が特定の方向に偏っている");
console.log("「異方性 (Anisotropy)」をオンラインPCAを用いて補正するシミュレーションです。\n");

const dim = 1536; // ada-002と同じ次元数
const numDocs = 1000;
const adapter = new WhiteningAdapter(dim, { learningRate: 0.01, numComponents: 1 });
...
console.log(`  -> 巨大なバイアスのせいで、全く関係ない文なのに類似度が極めて高くなってしまう！\n`);
...
console.log(`✅ Whitening後 (補正ベクトル) の類似度: ${whitenedSim.toFixed(4)}`);
console.log(`  -> 偏り成分が除去され、ノイズ同士（本来の無関係さ）が正しく低い類似度として反映された！\n`);

console.log("💡 結論: WhiteningAdapterを通すだけで、事前学習モデルの検索精度（解像度）をエッジ上で劇的に高めることができます。");
