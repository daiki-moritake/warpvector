import { IntentAdapter, normalize } from '../src';
...
console.log("学生向けにワープされたベクトル:", Array.from(normalize(studentQuery)));

// 5. ゲーマー向けの検索結果に最適化（ワープ）
const gamerQuery = adapter.tune(queryVector, "gamer");
console.log("ゲーマー向けにワープされたベクトル:", Array.from(normalize(gamerQuery)));
