...
const projector = new ProjectionAdapter(4, 2, {
  myProjection: projectionWeights,
});
...
// 4. 次元を圧縮して投影する
const newDimVector = projector.tune(oldDimVector, "myProjection");

console.log(
  `投影後のベクトル (${newDimVector.length}次元):`,
  Array.from(newDimVector),
);
