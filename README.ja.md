...
- 🔄 **ゼロダウンタイム移行**: モデル変更時（例: 1536次元から512次元へ）でもベクトルDBの再インデックスを不要にし、ベンダーロックインを完全に解消します。
...
- 💾 **4. ベクトルDBのメモリコストを 1/4 〜 1/32 に激減させる**
  ...
  > パイプラインの最後に `.setFinalStage("quantize", ...)` 等を追加するだけで、精度をほぼ落とさずにデータサイズを大幅に圧縮します。

- 🔄 **5. ゼロダウンタイム・モデル移行（再インデックス不要）**

  > `ada-002` から `text-embedding-3` に乗り換えますか？わずか100件のデータペアから `AlignmentAdapter` を学習させるだけで、新しいクエリを古いベクトル空間へ瞬時に翻訳し、ベンダーロックインを打破します。

- 🚀 **6. 現在の TypeScript コードを壊さずに「数行」で導入**
...
_Apple M-series, Bun ランタイムで計測。`bun run benchmarks/accuracy.ts` で再現可能。_
...
| 10  | [次元削減・モデル間移行](./docs/10-alignment-and-migration.ja.md)            | MigrationTrainer によるゼロダウンタイム空間翻訳                  |
...
- 🔄 [**ベクトルDBの再インデックス地獄を回避！TypeScriptで実現する「ゼロダウンタイム・モデル移行」**](https://zenn.dev/daiki_moritake/articles/zero-downtime-vector-migration)
