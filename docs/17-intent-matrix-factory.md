...
factory.addCategory("tech", [
  await embed("TypeScript runtime performance optimization"),
  await embed("WebAssembly SIMD vectorization techniques"),
  await embed("Edge computing latency reduction"),
  await embed("Bun vs Node.js benchmark comparison"),
  await embed("Zero-copy memory buffer management"),
]);

factory.addCategory("business", [
  await embed("Q4 revenue forecast and growth projections"),
  await embed("Customer acquisition cost optimization"),
  await embed("Market share analysis in SaaS industry"),
  await embed("Competitive landscape and positioning strategy"),
  await embed("Annual recurring revenue trends"),
]);
...
| `temperature` | `number` | `0.07` | InfoNCE の温度パラメータ |
...
// ✅ 良い: 多様な表現
factory.addCategory("tech", [
  await embed("TypeScript performance"),      // 一般的
  await embed("How to optimize WASM SIMD"),    // How-to 形式
  await embed("Node.js vs Bun benchmark"),     // 比較形式
  await embed("WebSocket latency profiling"),  // 具体的ツール
]);

// ❌ 悪い: ほぼ同じ表現の繰り返し
factory.addCategory("tech", [
  await embed("TypeScript performance"),
  await embed("TypeScript speed"),
  await embed("TypeScript optimization"),
]);
```
...
- [Trainers](./7-trainers.md) — オンライン学習エンジンの詳細
- [Auto-Learning Guide](./auto-learning-guide.md) — フィードバックループの実装ガイド
- [Integrations](./8-integrations.md) — LangChain / LlamaIndex との統合
