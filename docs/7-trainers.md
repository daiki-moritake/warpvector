...
// 学習率(Learning Rate)などを指定してトレーナーを初期化
const trainer = new TripletTrainer(1536, { learningRate: 0.001 });
...
// anchor: 検索クエリのベクトル
// positive: クリックされた正解ドキュメントのベクトル
// negative: 無視されたドキュメントのベクトル
const loss = trainer.updateOnline(
  anchorVector, 
  positiveVector, 
  negativeVector, 
  currentWeights // このオブジェクト内の値が破壊的に（インプレースで）更新されます
);

console.log(`Current Loss: ${loss}`);

// 更新された currentWeights を IntentAdapter に渡して検索に使う！
```
...
const loss = trainer.updateOnline(
  anchorVector, 
  positiveVector, 
  [negativeVector1, negativeVector2, negativeVector3], // 複数指定可能
  currentWeights
);
```
